...
# カラースキームの定義
TEAL_BLUE = RGBColor(14, 124, 134)      # #0E7C86 (主色)
LIGHT_GRAY = RGBColor(245, 247, 249)    # #F5F7F9 (補色)
LEAF_GREEN = RGBColor(76, 175, 80)      # #4CAF50 (アクセント1)
GRAPHITE = RGBColor(66, 66, 66)         # #424242 (アクセント2)
AMBER = RGBColor(255, 193, 7)           # #FFC107 (強調色)
DARK_GRAY = RGBColor(51, 51, 51)        # #333333 (本文)
WHITE = RGBColor(255, 255, 255)         # #FFFFFF (白)

# タイポグラフィサイズの定義 - サイズダウン
TITLE_SIZE = Pt(40)  # 44から40に縮小
HEADING_SIZE = Pt(32)  # 36から32に縮小
SUB_HEADING_SIZE = Pt(24)  # 28から24に縮小
BODY_SIZE = Pt(20)  # 24から20に縮小
SMALL_SIZE = Pt(18)  # 20から18に縮小
...
def add_treatment_methods_slide(prs):
    ...
    x, y, cx, cy = Inches(1.5), Inches(1.7), Inches(7), Inches(2.3)
    chart = slide.shapes.add_chart(
    ...
    # テキストボックスの位置調整
    left = Inches(1.5)
    top = Inches(4.2)  # 4.5から4.2に変更
    width = Inches(7)
    height = Inches(2.5)  # 2から2.5に拡大
    textbox = slide.shapes.add_textbox(left, top, width, height)
    tf = textbox.text_frame
...
def add_government_regulations_slide(prs):
    ...
    # 左側: 規制の影響 - 位置調整
    left = Inches(0.8)  # 1から0.8に調整
    top = Inches(1.7)  # 1.8から1.7に調整
    width = Inches(4.0)  # 4.5から4.0に調整
    height = Inches(0.4)
    textbox = slide.shapes.add_textbox(left, top, width, height)
    tf = textbox.text_frame
    tf.text = "規制の影響:"
    ...
    # 中央の区切り線 - 位置調整
    left = Inches(4.9)  # 5から4.9に調整
    top = Inches(1.9)  # 2から1.9に調整
    width = Inches(0.02)
    height = Inches(4)
    line = slide.shapes.add_shape(
        1, left, top, width, height
